...
class FriendsQuery extends DBQuery<DBFriend[]> {
  ...
  public override readonly query = `
    SELECT users.userid, users.username, users.league, users.highest_score, users.trophies, users.puzzle_elo 
    FROM users
...
export class GetFriendsInfoRoute extends GetRoute<FriendInfo[]> {
  ...
  override async get(userInfo: UserInfo | undefined): Promise<FriendInfo[]> {
    ...
    const friendsInfo = await Database.query(FriendsQuery, userInfo!.userid);
    
    return friendsInfo.map((friend) => ({
        userid: friend.userid,
        username: friend.username,
        isOnline: users.isUserOnline(friend.userid),
        activity: users.getUserActivityType(friend.userid),
